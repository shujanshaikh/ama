...
export const DiffReviewPanel = memo(function DiffReviewPanel({
    ...
    for (const message of messages) {
        ...
        for (const part of message.parts) {
            if (part.type === 'tool-editFile' && part.state === 'output-available') {
                ...
                if (filePath && (output?.old_string || output?.new_string) && output?.old_string !== output?.new_string) {
                    diffItems.push({
                        id: part.toolCallId,
                        filePath,
            ...
            if (part.type === 'tool-stringReplace' && part.state === 'output-available') {
        ...
        return (
            <div className="flex flex-col h-full bg-background">
                ...
                <div className="flex-1 flex items-center justify-center">
                    <p className="text-sm text-muted-foreground">No file changes to review</p>
                </div>
            </div>
        );
    }
