...
import { FileText, Code, Eye, ChevronDown, ChevronUp, Maximize2 } from 'lucide-react';
...
export const MarkdownEditor = memo(({ fileName, content, isStreaming }: MarkdownEditorProps) => {
      ...
      {lines.map((line, idx) => (
        <div key={idx} className="flex hover:bg-muted/50 rounded-sm">
          ...
          <span className={cn(
            ...
            line.startsWith('#') ? 'text-foreground font-medium' : 'text-foreground/70'
          )}>
            {line || '\u00A0'}
          </span>
        </div>
