...
async function readPlanFile(planName: string): Promise<string | null> {
	try {
		const planFilePath = `.ama/plan.${planName}.md`;
		const planResult = await executeTool("readFile", {
...
export async function buildPlanSystemPrompt(
	...
	// Handle plan execution: read plan file and include in context
	if (executePlan && planName && projectCwd) {
		const planContent = await readPlanFile(planName);
		if (planContent) {
			return buildPlanExecutionPrompt(planContent);
		}
	}
	
	// Handle plan creation: modify system prompt
	if (planMode) {
